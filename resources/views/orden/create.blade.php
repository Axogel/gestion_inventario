    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('orden.store') }}">
                ...
                <button type="submit" id="btn-submit-order" class="btn btn-primary mt-3">
...
@section('js')
    ...
    <script>
        ...
        let rowIndex = 0;
        let paymentIndex = 0;
        const TOLERANCE = 0.10; // 10 centavos
        ...
        function calculatePayments() {
            ...
            let diff = total - totalCOP;

            // Si la diferencia es menor a la tolerancia, se ajusta
            ...
            $('#total-paid').text(total.toFixed(2));
            $('#change').text(diff > 0 ? diff.toFixed(2) : '0.00');

            let remainingCOP = diff < 0 ? Math.abs(diff) : 0;
        ...
        $('form').on('submit', function (e) {
            ...
            let rows = $('#payments-table tbody tr');
            ...
            if (diff < -TOLERANCE) {
                if (!clientId && !isNewClient) {
                    e.preventDefault();
                    alert('❌ Para dejar una orden fiada debes seleccionar un cliente');
                    return;
                }
            ...
            $('#services-table tbody tr').each(function () {
                ...
                if (!serviceId || price <= 0) valid = false;
            });
            ...
            $('#products-table tbody tr').each(function () {
                ...
                if (productSelect.length && !productSelect.val()) valid = false;
            });
